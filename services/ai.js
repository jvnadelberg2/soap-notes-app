'use strict';
// services/ai.js — Ollama JSON adapter (no fallback text), logs RAW
...
function buildJSONPrompt({ subjective = '', objective = '', specialty = '', assistLevel = 1, noteType = 'SOAP' }) {
  const detail =
    assistLevel >= 3 ? 'Write an expanded, clinically useful Assessment and Plan with brief rationale and specific next steps.' :
...
async function callOllama({ model, prompt }) {
  ...
  console.log('[ai] calling ollama model=', m);
  ...
  console.log('[ai] ollama status', res.status);
  if (!res.ok) throw new Error(`Ollama HTTP ${res.status}`);
  const data = await res.json();
  const text = (data && (data.response || data.text)) || '';
  console.log('[ai] RAW <<<', (text || '').slice(0, 600), '>>>');
...
// ---------- EXPORTS (canonical + aliases) ----------
module.exports = { generate };
module.exports.generateSoapNote = module.exports.generate; // legacy-safe
