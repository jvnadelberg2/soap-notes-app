...
router.post("/generate-soap-json", async (req, res) => {
  try {
    ...
    const note = await generateSoapNoteJSON({ rawText, patientHistory, specialty, vitals, labs, imaging, allowInference, model, provider });
    res.json(note);
  } catch (e) {
    console.error(e);
...
router.post("/generate-soap", async (req, res) => {
  try {
    const { rawText = "", patientHistory = "", specialty = "General Practice", vitals = null, labs = null, imaging = null, allowInference = false, model = null, provider = "ollama" } = req.body || {};
    const text = await generateSoapNoteText({ rawText, patientHistory, specialty, vitals, labs, imaging, allowInference, model, provider });
