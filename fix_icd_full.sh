...
function parse(line){
  ...
  if(i<1)return null;
  let code=line.slice(0,i).trim(),desc=line.slice(i+1).trim();
  if(code[0]==='"'&&code.at(-1)==='"')code=code.slice(1,-1);
  if(desc[0]==='"'&&desc.at(-1)==='"')desc=desc.slice(1,-1);
...
export function lookupICD(code){
  const M=load();const c=String(code||"").toUpperCase().trim();
  return c?(M[c]||null):null
}
export function findBestICD({text,limit=10}){
  const M=load();const q=toks(text);
  if(!q.length)return[];
  const out=[];
  for(const [code,desc]of Object.entries(M)){
    const c=code.toLowerCase(),d=desc.toLowerCase();let s=0;
    for(const t of q){if(c===t)s+=120;else if(c.startsWith(t))s+=60;if(d.includes(t))s+=15}
    if(s>0)out.push({code,desc,score:s})
  }
...
export function icdDebug(){
  const M=load();
  return{file:SRC||null,size:Object.keys(M).length,sample:{I10:M["I10"]||null,"R51.9":M["R51.9"]||null}}
}
EOT
...
router.post("/generate-soap-json-annotated",async(req,res)=>{
  try{
    const{rawText="",patientHistory="",specialty="General Practice",vitals={},labs={},imaging=[],allowInference=false,model=null,provider="ollama"}=req.body||{};
    const data=await generateSoapNoteJSON({rawText,patientHistory,specialty,vitals,labs,imaging,allowInference,model,provider});
    const parts=[rawText,patientHistory,data?.subjective?.rawText||"",data?.objective?.text||data?.objective||"",data?.assessment?.text||data?.assessment||"",data?.plan?.text||data?.plan||""].filter(Boolean).join(" ");
    const icd=findBestICD({text:parts,limit:12})||[];
    res.json({data,icd})
...
node -e "const fs=require('fs');let s='';try{s=fs.readFileSync('index.html','utf8')}catch{}if(s){s=s.replace(/icdOut\.textContent\s*=\s*.*?;\s*$/m,'icdOut.textContent = (j.icd&&j.icd.length)? j.icd.map(x=>(x.code||\"\")+\" — \"+((x.desc||x.term)||\"\")).join(\"\\n\") : \"No ICD-10 codes detected.\";\\n');fs.writeFileSync('index.html',s)}"
