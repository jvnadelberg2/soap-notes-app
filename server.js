...
app.get("/api/models", async (req, res) => {
  ...
  try {
    ...
    if (r.ok) {
      ...
      // data.models is usually [{name, size, ...}]
  ...
  try {
    // try absolute path first, then rely on PATH
...
function computeSubjective(body){
  ...
  const psh       = s(body.psh);                // ✅ added
  const fh        = s(body.familyHistory);      // ✅ fixed
  const sh        = s(body.socialHistory);      // ✅ fixed
  const ros       = s(body.ros);
  const allergies = s(body.allergies);          // ✅ moved here
...
async function handleSoap(req, res){
	try{
		...
		const assistLevel = Number.isFinite(lvlRaw) ? Math.max(0, Math.min(4, lvlRaw)) : 1;
