...
let T=null,SRC=null;
function findFile(){for(const p of [["public","icd10cm.csv"],["public","icd10cm.txt"],["data","icd10cm.csv"],["data","icd10cm.txt"]]){const f=path.join(__dirname,"..","..",...p); if(fs.existsSync(f)) return f}
...
function load(){ if(T) return T; SRC=findFile(); const map=Object.create(null); if(SRC){const txt=fs.readFileSync(SRC,"utf8"); for(const ln of txt.split(/\r?\n/)){ if(!ln.trim()) continue; const r=parse(ln); if(r && !map[r.code]) map[r.code]=r.desc }} T=map; return T }
...
export function findBestICD({text,limit=10}){const M=load(); const q=toks(text); if(!q.length) return []; const out=[]; for(const [code,desc] of Object.entries(M)){const c=code.toLowerCase(), d=desc.toLowerCase(); let s=0; for(const t of q){ if(c===t) s+=120; else if(c.startsWith(t)) s+=60; if(d.includes(t)) s+=15 } if(s>0) out.push({code,desc,score:s}) } out.sort((a,b)=>b.score-a.score); return out.slice(0,Math.max(1,Math.min(200,parseInt(limit,10)||10))).map(x=>({code:x.code,desc:x.desc}))}
export function icdDebug(){const M=load(); return {file:SRC||null,size:Object.keys(M).length,sample:{I10:M["I10"]||null,"R51.9":M["R51.9"]||null}}}
EOT
...
perl -0777 -pe 's/(const\s+icd\s*=\s*j\.icd[^\n]*\n[^\n]*icdOut[^\n]*=)[^\n]*;/\1 icd&&icd.length?icd.map(x=>`${"${"}x.code}${" — "}${"${"}(x.desc||x.term||"")}`).join("\\n"):"No ICD-10 codes detected.";/s' -i '' index.html 2>/dev/null || true
