(function(){
  ...
  function showICD(list){
    ...
    box.innerHTML='<ul>'+list.map(x=>`<li><b>${x.code}</b> — ${x.term} <span class="small">(score ${x.score})</span></li>`).join('')+'</ul>';
  }
  ...
  function stripJsonWords(){
    document.querySelectorAll('h1,h2,h3').forEach(h=>{
      h.textContent = h.textContent.replace(/SOAP\s*JSON/ig,'SOAP Note').replace(/\bJSON\b/ig,'Note');
    });
    const gen=el('genJson'); if(gen) gen.textContent='Generate Note';
    const stream=el('genStream'); if(stream) stream.textContent='Stream Note';
  ...
  document.addEventListener('DOMContentLoaded', ()=>{
    ...
    const g1=replaceButton('genJson');
    const g2=replaceButton('genStream');
    if(g1) g1.addEventListener('click', (e)=>{e.preventDefault(); generate();}, {capture:true});
    if(g2) g2.addEventListener('click', (e)=>{e.preventDefault(); generate();}, {capture:true});

    const hpi=el('rawText');
    const hx=el('patientHistory');
    if(hpi) hpi.addEventListener('blur', autofillFromHPI);
    if(hx)  hx.addEventListener('blur', autofillFromHPI);
